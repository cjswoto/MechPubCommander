...
class SettingsDialog(simpledialog.Dialog):
    def __init__(self, parent, title, username="", tesseract_path="", team_file="", enemy_file=""):
    ...
    def body(self, master):
        ...
        self.browse_tesseract_button.grid(row=1, column=2)

        tk.Label(master, text="Team File:").grid(row=2)
        self.team_file_entry = tk.Entry(master)
        self.team_file_entry.grid(row=2, column=1)
        ...
        self.browse_team_button.grid(row=2, column=2)

        tk.Label(master, text="Enemy File:").grid(row=3)
        self.enemy_file_entry = tk.Entry(master)
        self.enemy_file_entry.grid(row=3, column=1)
        ...
        self.browse_enemy_button.grid(row=3, column=2)
    ...
    def apply(self):
        self.result = (self.name_entry.get(), self.tesseract_entry.get(),
                       self.team_file_entry.get(), self.enemy_file_entry.get())
...
class OverlayWindow:
    def __init__(self):
        ...
        script_dir = resource_path('')  # Base directory of the executable
    ...
    def auto_login(self):
        ...
        if email and password:
            ...
            response = self.session.post(login_url, data=login_data)
            logging.info(f"Auto-login attempt for email: {email}")
            logging.debug(f"Auto-login response status code: {response.status_code}")
            logging.debug(f"Auto-login response headers: {response.headers}")
    ...
    def open_settings(self):
        dialog = SettingsDialog(self.root, "User Settings", self.username, self.tesseract_path, self.team_file,
                                self.enemy_file)
        if dialog.result:
            self.username, self.tesseract_path, self.team_file, self.enemy_file = dialog.result
            self.save_settings()
            self.check_tesseract_installation()
            self.populate_friend_list()
    ...
    def import_image(self):
        subprocess.Popen(r'C:\PUGCommander\ImageParserApp.exe')
